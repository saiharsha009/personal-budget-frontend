...
const BudgetForm = (props) => {
  ...
  const handleDataSubmition = async () => {
    ...
    setCalculatedAmmount(parseInt(calculatedAmmount) + parseInt(catagoryBudget));
    setChipData(newChipObject);
    if(dataForBudget && dataForBudget.length!=0) {
  ...
  return (<>
    <div style={{ padding: "30px", borderRadius: "10px", margin: "10px", border: "2px dotted #3b71ca" }}>
      ...
      <MDBInput type='text' value={catagory} onChange={(e) => setCatagory(e.target.value)} id='form4Example2' wrapperClass='mb-4' label='Name your category' />
      <MDBInput type='number' value={catagoryBudget} onChange={(e) => setCatagoryBudget(e.target.value)} id='form4Example2' wrapperClass='mb-4' label='Set Category Amount' />
      {/* <MDBBtn onClick={} type='submit' className='mb-4' disabled={!dataForBudget || dataForBudget.length === 0} >
        ADD CATEGORY
      </MDBBtn> */}
      <MDBBtn onClick={() => handleDataSubmition()} type='submit' style={{ margin: '0' }} block>
        Submit
      </MDBBtn>
    </div>
  </>
